<div class="usercard-message">
  <div class="mdui-btn-group">
    <input type="hidden" data_id='{{ $user->id }}' >
    @guest
    ...
    @else
      ...
      @if (Auth::user()->isFollowing($user->id))
        <button class="mdui-btn mdui-ripple club-unfollower" title="取消关注将不会再收到他的动态"><i class="mdui-icon material-icons">&#xe5ca;</i> 已关注</button>
      @else
        <button class="mdui-btn mdui-ripple club-follower" title="关注后能收到他的最新动态"><i class="mdui-icon material-icons">&#xe145;</i> 加关注</button>
      @endif
      @endif
      @endguest
    </div>