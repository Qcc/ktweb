@extends('layouts.app')
@section('title', isset($solution->id) ? $solution->title : '新建资讯')
@section('content')
            ...
            <form action="{{ route('solution.update', $solution->id) }}" method="POST" accept-charset="UTF-8">
                ...
                <form action="{{ route('solution.store') }}" method="POST" accept-charset="UTF-8">
                    ...
                    <div class="form-group select">
                        <select class="mdui-select" mdui-select name="solution_id" required>
                            <option value="" hidden disabled {{ $solution->id ? '' : 'selected' }}>请选择分类</option>
                            @foreach ($solutioncol as $value)
                            <option value="{{ $value->id }}" {{ $solution->solution_id == $value->id ? 'selected' : '' }}>
                                {{$value->name }}</option>
                            @endforeach
                        </select>
                    </div>
                    ...
                    <div class="form-group title">
                        <input class="form-control" type="text" name="image" value="{{ old('image', $solution->image ) }}"
